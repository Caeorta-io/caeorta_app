...
|---|---|---|---|
| 2026-07-03 | **Mobile design-system foundation: DARK is the default and only live theme; LIGHT is committed as a re-skin (`colorsLight`) but not wired.** Semantic tokens (§4.2) are translated once into `apps/mobile/design/tokens.js` — a plain-CommonJS single source required by BOTH `tailwind.config.js` (className scales) and the typed `src/design/` layer (via a hand-written `tokens.d.ts`) — and bound **statically** into `theme.extend.colors`, NOT via CSS variables. Radius is re-defined to the design scale (`rounded-xl` 12→20 etc.): a config-only change the un-migrated Week 1–3 screens inherit until the Week 8 pass (forward-only). | Founder chose "static dark, light captured for later" at the ASK gate over NativeWind CSS-var two-mode theming, given the react-native-css-interop fragility logged in earlier sessions — static dark has no runtime-resolution risk. The Figma flip-a-mode model is preserved structurally: `colorsLight` mirrors `colorsDark` key-for-key (test-enforced), so turning light on later is a config swap with zero screen edits. | `apps/mobile/design/tokens.js` + `tokens.d.ts`; `apps/mobile/tailwind.config.js`; `apps/mobile/src/design/*`; `docs/conventions.md` → "Design system"; this row + session 26 diary |
...
- **`docs/conventions.md`:** new **"Design system"** section — token→className map, dark-default/light-re-skin policy, forward-only migration policy (incl. the radius-override caveat), typography-by-name, icon convention, pressed-opacity/48dp, no-raw-hex-in-new-code.
...
- **Week 1–3 restyle** — old screens stay on stock classes until the Week 8 polish pass; note they inherit the new radius scale in the meantime.
