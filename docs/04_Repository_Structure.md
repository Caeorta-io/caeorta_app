# Repository Structure & Environments

## Monorepo layout

Single repository: `caeorta_app`. Hosted on GitHub under the **Caeorta-io** organization (not personal accounts).

> **This tree is the actual layout**, verified against `origin/main` (2026-08-03). It was
> substantially aspirational until then — it described `apps/mobile/app|components|hooks|lib`
> at the app root when they live under `src/`, an `apps/admin/components/` and a
> `packages/ui/` that never existed, and four `docs/*.md` files that were never created.
> **When the layout changes, update this block in the same PR** (CLAUDE.md § Workflow
> conventions). A tree that describes an intended structure rather than the real one is
> worse than no tree.

```
caeorta_app/
├── apps/
│   ├── mobile/                     # Expo (React Native) app — @caeorta/mobile
│   │   ├── src/                    # ALL source lives under src/, not the app root
│   │   │   ├── app/                # expo-router file-based routes
│   │   │   │   ├── _layout.tsx
│   │   │   │   ├── (auth)/         # sign-in, verify (email OTP)
│   │   │   │   └── (app)/          # authenticated stack
│   │   │   │       ├── index.tsx
│   │   │   │       ├── vehicles/   # list, add/, [id]/ → drives/, dtcs/, live, diagnostics
│   │   │   │       ├── pair/       # scan, manual, result
│   │   │   │       ├── wifi/       # index, credentials, result
│   │   │   │       └── dev/        # __DEV__ harnesses: diagnostic-card, telemetry, tokens
│   │   │   ├── components/         # UI components (incl. dtc/, ui/)
│   │   │   ├── hooks/              # TanStack Query hooks
│   │   │   ├── lib/                # data seam (lib/data/), derivations, stores, i18n
│   │   │   ├── design/             # typed token layer (consumes ../design/tokens.js)
│   │   │   ├── locales/            # i18n resources (en.json)
│   │   │   └── global.css          # NativeWind entry
│   │   ├── design/                 # tokens.js + tokens.d.ts — plain CommonJS, shared with
│   │   │                           #   tailwind.config.js (must stay outside src/)
│   │   ├── assets/                 # fonts (Geist), images
│   │   ├── app.json                # Expo config (plugins, scheme)
│   │   ├── eas.json
│   │   ├── tailwind.config.js
│   │   ├── metro.config.js
│   │   ├── babel.config.js
│   │   ├── vitest.config.ts
│   │   ├── eslint.config.js
│   │   ├── package.json
│   │   └── tsconfig.json
│   │
│   ├── admin/                      # Next.js 16 admin dashboard — @caeorta/admin
│   │   ├── app/                    # App Router (no src/ here — Next convention)
│   │   │   ├── layout.tsx
│   │   │   ├── page.tsx            # device list
│   │   │   ├── devices/[id]/       # device detail: drives + DTC timeline
│   │   │   ├── login/
│   │   │   ├── auth/callback/      # route handler
│   │   │   └── globals.css
│   │   ├── lib/
│   │   │   ├── supabase/           # client.ts (browser) + server.ts (RSC)
│   │   │   └── utils.ts
│   │   ├── public/
│   │   ├── components.json         # shadcn/ui config FILE (not a components/ dir)
│   │   ├── proxy.ts
│   │   ├── next.config.ts
│   │   ├── postcss.config.mjs
│   │   ├── eslint.config.mjs
│   │   ├── package.json
│   │   └── tsconfig.json
│   │
│   └── marketing/                  # Reserved path; Framer hosts the actual site.
│       └── README.md               # No package.json — NOT a pnpm workspace member
│
├── packages/
│   ├── types/                      # Shared domain types + zod schemas — @caeorta/types
│   │   ├── src/
│   │   │   ├── device.ts   diagnostic.ts   drive.ts   dtc.ts
│   │   │   ├── provisioning.ts   sync.ts   telemetry.ts   vehicle.ts
│   │   │   ├── __tests__/
│   │   │   └── index.ts
│   │   ├── eslint.config.mjs
│   │   ├── package.json
│   │   └── tsconfig.json
│   │
│   ├── supabase/                   # Shared client + generated DB types — @caeorta/supabase
│   │   ├── src/
│   │   │   ├── client.ts           # Universal client factory
│   │   │   ├── realtime.ts         # subscribeTo* helpers + unsubscribe
│   │   │   ├── database.types.ts   # Auto-generated (`pnpm --filter @caeorta/supabase gen:types`)
│   │   │   └── index.ts            # re-exports Tables<>, Json, Database, auth types
│   │   ├── eslint.config.mjs
│   │   ├── package.json
│   │   └── tsconfig.json
│   │
│   └── config/                     # Shared ESLint, Prettier, TS configs — @caeorta/config
│       ├── eslint-config/
│       ├── prettier-config/
│       ├── tsconfig/
│       └── package.json
│
├── supabase/                       # Platform-track: migrations, Edge Functions, seed
│   ├── migrations/                 # Timestamped SQL, immutable once applied
│   │   ├── 20260602125801_enable_extensions.sql
│   │   ├── 20260602130000_initial_schema.sql
│   │   ├── 20260602150000_rls_policies.sql
│   │   ├── 20260614000001_add_notify_agent.sql
│   │   ├── 20260614000002_add_pg_cron_jobs.sql
│   │   ├── 20260615000001_dtcs_add_freeze_frame_metrics.sql
│   │   └── 20260615000002_add_dtc_lookup_table.sql
│   ├── functions/                  # Edge Functions (Deno) — 12 + _shared/
│   │   ├── _shared/                # cors.ts, errors.ts
│   │   ├── pair_device/   mint_device_token/   submit_wifi_credentials/
│   │   ├── ota_check/     create_vehicle/      update_current_state/
│   │   ├── device_sync_start/   device_sync_chunk/   device_sync_complete/
│   │   ├── get_drive_telemetry/
│   │   └── send_diagnostic_notification/
│   ├── seed.sql                    # Dev fixtures (users, vehicles, devices, telemetry)
│   ├── seed_dtc_lookup.sql         # 52 common P0xxx codes
│   └── .gitignore                  # NOTE: no config.toml — the CLI is used --linked
│
├── docs/
│   ├── 00_README.md … 11_Carry_Forwards.md   # the numbered canon (00–11)
│   ├── conventions.md              # Cross-cutting working patterns
│   ├── workdiary.md                # Living session log + decisions log
│   ├── create_vehicle_contract.md  # Edge Function wire contract
│   ├── adr/                        # Architecture Decision Records
│   │   ├── 0001-monorepo-with-pnpm-workspaces.md
│   │   └── 0002-expo-router-stack-protected-for-auth-guards.md
│   ├── design/00_design_system.md  # Designer-owned; §-referenced throughout
│   └── testing/device_simulation.sh
│
├── .github/
│   ├── CODEOWNERS                  # Routes Platform-area paths to the Platform founder
│   └── workflows/
│       ├── ci.yml                  # Lint, typecheck, test on every PR
│       └── mobile-preview.yml      # EAS preview build on PR
│
├── .gitattributes
├── .gitignore
├── .nvmrc                          # Node version pinned (22 LTS)
├── CLAUDE.md                       # Claude Code's project brief (read every session)
├── README.md
├── package.json                    # Root; pnpm workspaces + onlyBuiltDependencies
├── pnpm-workspace.yaml             # apps/*, packages/* + allowBuilds
├── pnpm-lock.yaml
└── tsconfig.base.json
```

**Workspace membership.** `pnpm-workspace.yaml` globs `apps/*` and `packages/*`, but a
directory is only a workspace member if it has a `package.json`. `apps/marketing` has only
a README, so there are **five** workspace packages plus the root — which is why the gate
reports `Scope: 5 of 6 workspace projects`.

## Workspace package conventions

Every workspace package has:

- `package.json` with `"name": "@caeorta/<name>"`
- `tsconfig.json` extending `tsconfig.base.json`
- A `src/` directory for source
- A `README.md` explaining what the package is for and its primary exports

Apps reference packages via workspace protocol:
```json
"dependencies": {
  "@caeorta/types": "workspace:*",
  "@caeorta/supabase": "workspace:*"
}
```

### Monorepo-wide version pinning via pnpm.overrides

When a dependency must be pinned across all workspaces (e.g., TypeScript, where mismatched versions in apps and packages cause subtle type-checking divergence), use `pnpm.overrides` in the root package.json:

```json
{
  "pnpm": {
    "overrides": {
      "typescript": "5.9.3"
    }
  }
}
```

This enforces the version at install time regardless of what individual workspaces specify. Use sparingly; only when a real cross-workspace consistency requirement exists. Current overrides at time of writing: `typescript: 5.9.3` (pinned in session 9 to standardize across Expo 56, Next.js 16, and packages).

## Branch strategy

- `main` is always deployable. Web deploys automatically from main; mobile uses EAS Update for JS-only changes.
- Feature branches: `feat/short-description`, `fix/short-description`, `chore/short-description`
- PRs in the App area are reviewed by Sulaiman before merge. PRs in the Platform area are reviewed by Sulaiman before merge. Both founders must agree on cross-cutting changes (schema, agent contract, CI architecture).
- Squash merge into main
- No direct commits to main except in genuine emergencies; document if it happens

**Repo settings — APPLIED 2026-08-03** (they had been documented as intended but never
actually enabled, which is the mechanism behind R19's four recurrences; see `docs/11`
§ CF-20). Current state, verified via `gh api repos/Caeorta-io/caeorta_app`:

| Setting | Value |
|---|---|
| `allow_squash_merge` | `true` |
| `allow_merge_commit` | **`false`** |
| `allow_rebase_merge` | **`false`** |
| `delete_branch_on_merge` | **`true`** |

Squash is now the only available merge method in the UI, and merged head branches are
deleted automatically. (A one-time cleanup on 2026-08-03 removed 43 stale branches left
behind from before `delete_branch_on_merge` was on; all had merged PRs and are restorable
from their PR pages.)

**Reviewer routing:** `.github/CODEOWNERS` auto-requests the owning founder per path —
Platform-area paths (`supabase/`, `apps/admin/`, `packages/supabase/`, `.github/workflows/`)
and the cross-cutting docs (`05`, `06`, `08`, `09`, `11`, `AI_Agent_Contract/`) route to
both founders; everything else defaults to the App founder.

**What is still NOT enforced:** branch protection on `main` (required status checks) is
unavailable on the GitHub Free plan, so `ci.yml`'s lint/typecheck/test results appear on
every PR but **cannot be marked required** — a red run does not block merging, and
CODEOWNERS review cannot be made mandatory either. Enforcement therefore remains
honour-system, per `docs/02_Working_Agreements.md`: do not merge a PR with failing CI, and
Sulaiman's review stays the human gate before `main`. Branch protection is funding-gated
(post-pilot, paid plan).

## Commit conventions

[Conventional Commits](https://www.conventionalcommits.org/):

- `feat:` new feature
- `fix:` bug fix
- `chore:` tooling, dependencies, cleanup
- `docs:` documentation only
- `refactor:` code restructuring without behavior change
- `test:` test additions or changes
- `perf:` performance improvement

Examples:
- `feat(mobile): add QR scanner to pairing flow`
- `fix(admin): correct device offline timestamp display`
- `chore: bump expo to SDK 56.0.5`

## Environments

Three environments. Strictly separated.

### Local development
- Muhammed runs Expo Go on his phone (or simulator/emulator) hitting his local machine
- Local machine connects to **dev Supabase**
- Edge Functions tested locally via `supabase functions serve`
- Migrations applied to dev Supabase via `supabase db push`

### Preview
- EAS preview builds (TestFlight internal-internal use, ad-hoc Android APKs)
- Hits **dev Supabase**
- Used for designer reviews, integration tests with AI agent project, internal demos

### Production
- Built only from `main` branch
- Hits **prod Supabase**
- Distributed via TestFlight (iOS) and Play Internal Testing (Android) to pilot users
- Sentry release tracking enabled
- PostHog production project

## Environment variables

### Mobile app (`apps/mobile`)

```
EXPO_PUBLIC_SUPABASE_URL
EXPO_PUBLIC_SUPABASE_ANON_KEY
EXPO_PUBLIC_SENTRY_DSN
EXPO_PUBLIC_POSTHOG_API_KEY
EXPO_PUBLIC_POSTHOG_HOST
EXPO_PUBLIC_APP_ENV          # dev | preview | prod
```

Set via:
- `.env.local` for local dev (gitignored)
- EAS environment variables (set via EAS dashboard) for preview and prod builds

### Admin web (`apps/admin`)

```
NEXT_PUBLIC_SUPABASE_URL
NEXT_PUBLIC_SUPABASE_ANON_KEY
SUPABASE_SERVICE_ROLE_KEY    # Server-side only, never exposed to client
NEXT_PUBLIC_APP_ENV
NEXT_PUBLIC_SENTRY_DSN
SENTRY_AUTH_TOKEN            # For sourcemap upload
NEXT_PUBLIC_POSTHOG_API_KEY
NEXT_PUBLIC_POSTHOG_HOST
RESEND_API_KEY               # For transactional email
```

Set via Vercel dashboard for preview and prod.

### Supabase Edge Functions

Secrets (set via `supabase secrets set`):
```
EXPO_ACCESS_TOKEN            # For Expo Push API
ANTHROPIC_API_KEY            # If any direct LLM calls from these functions (typically no — AI agent owns this)
DEVICE_JWT_SIGNING_SECRET    # For minting device tokens
WIFI_CREDENTIAL_ENCRYPTION_KEY  # For encrypting stored Wi-Fi credentials
```

## Secret management

- All production secrets stored in 1Password (or equivalent), shared vault between two founders
- API keys rotated quarterly; date of last rotation tracked in 1Password
- Apple signing certificates and Google signing keys stored in 1Password
- **Critical: losing Google's app signing key means losing the ability to update the Play Store listing forever.** Treat this with extreme care.

## Node and pnpm versions

Pinned via `.nvmrc` and `package.json` engines:
```
node: 22.x LTS
pnpm: 11.x (latest stable; managed via corepack, not installed globally)
```

Versions are pinned so Muhammed's local environment matches CI and any future contributor inherits the same setup.

## CI/CD pipeline

GitHub Actions handles:

1. **On every PR:**
   - Lint (ESLint)
   - Typecheck (tsc --noEmit)
   - Unit tests (Vitest)
   - Build mobile (EAS preview build, optional, triggered by PR label)

2. **On merge to main:**
   - Web deploys automatically via Vercel
   - Mobile JS-only changes ship via EAS Update
   - Mobile native changes trigger EAS Build → EAS Submit to TestFlight + Play Internal Testing

3. **Manual workflows:**
   - Database migration verification (against a temp branch)
   - Edge function deployment (when functions change)

## Documentation discipline

`docs/` is part of the repository, version-controlled. Updates to docs go through PR review same as code.

- **Workdiary** (`docs/workdiary.md`) is the living log of every working
  session: tool inventory, decisions log, per-session entries. Read latest
  entry at session start; append entry at session end. Format template at
  the bottom of the file.
- **ADRs** (Architecture Decision Records) for any decision worth remembering: why we chose Expo, why we kept Supabase, why we deferred Peaq, etc. Format: short, numbered, dated, immutable (write a new ADR to supersede an old one).
- **API contracts** for every Edge Function: input shape, output shape, error cases, auth requirements.
- **Schema doc** is the human-readable companion to migrations. Updated in the same PR as a migration.
- **AI Agent Contract** is the single source of truth for the interface to the AI agent project.

## Local dev setup checklist

A new contributor (or Muhammed on a new machine) should be able to follow this and be running in under 30 minutes:

1. Install Node 22 (current Active LTS as of 2026; Node 20 went EOL 2026-04), pnpm 11
2. Clone repo
3. `pnpm install` at root
4. Install Supabase CLI
5. `supabase link --project-ref <dev-project-ref>`
6. Copy `.env.example` to `.env.local` in each app, fill in dev keys (from 1Password)
7. Install VS Code (free, from code.visualstudio.com)
8. Install the Claude Code extension from the VS Code Marketplace (official one by Anthropic)
9. Sign in with your Anthropic account when prompted
10. (Mobile) Install Expo Go on phone, scan QR from `pnpm --filter mobile dev`
11. (Web) `pnpm --filter admin dev` opens at localhost:3000

The `README.md` at the repo root contains exactly these steps.

## Known environmental gotchas

These surfaced during session 9's monorepo scaffold. Capture so they don't bite again on a fresh setup.

**pnpm 11 build-script config moved**. In pnpm 9, certain build-script approvals lived in package.json. In pnpm 11, they moved into `pnpm-workspace.yaml` under an `ignoredBuiltDependencies` or `onlyBuiltDependencies` key. Watch for unexpected "skipping build script" warnings during install; check the workspace file's documented schema, not the package.json one.

**Tailwind 4 oxide compiler corrupts paths containing `#`**. The Tailwind 4 oxide compiler used by NativeWind 4 and Next.js 15+ corrupts paths that contain a literal `#` character. The repo working directory is renamed to `1_Caeorta_dev` (not `#1_Caeorta_dev`) for this reason. If a future contributor uses a path with `#`, they will hit baffling CSS-not-found errors in admin. Fix: rename the directory.

**NativeWind under pnpm strict linker**. pnpm's strict module-resolution requires `react-native-css-interop@0.2.4` to be a direct dependency of `apps/mobile`, not a transitive one through NativeWind. Without it, you get module-not-found errors at Expo bundle time despite NativeWind appearing to install correctly.

**Expo Go's bundled SDK lags the latest by one major**. The Expo Go app on the Play Store ships with a specific Expo SDK version (SDK 54 as of session 11, 2026-06-09) and updates lag behind newer SDK releases by weeks to months. When this repo's Expo SDK is newer than Expo Go's bundled SDK (currently SDK 56 vs Expo Go's 54), Expo Go cannot run the app — it will refuse to load. Use an EAS development build for on-device testing instead. The dev build path also enables features Expo Go doesn't support (native Sentry capture, expo-dev-client menu, custom native modules). Cost: a fresh APK install per pilot device, instead of zero-install Expo Go testing.

## CLAUDE.md — Claude Code's project brief

`CLAUDE.md` at the repo root is read automatically by Claude Code (VS Code
extension and CLI) at the start of every session. It is a short brief — one
screen — that tells Claude what this project is, what stack it uses, where the
detailed docs live, what conventions to follow, and what's out of scope.

Keep it focused:
- The file is included in every Claude Code request, so length costs tokens
- Detailed content lives in `docs/` — `CLAUDE.md` points there, doesn't duplicate
- When you find yourself repeatedly correcting Claude on the same thing, add a
  line to `CLAUDE.md` so the next session starts knowing

Subfolder `CLAUDE.md` files (e.g., `apps/mobile/CLAUDE.md`) are supported for
area-specific conventions. Start with just the root one; add subfolder ones
only if a real need emerges.

Commit it to git so every Claude Code session in this repo (current and future) gets the same instructions.