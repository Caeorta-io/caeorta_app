...
### Week close — App track (sessions 25–29)

- **Week 4 App track complete** as the delivered design defines it (mock-data-first, except the one live telemetry read). Screens/foundation shipped: **drives list** (paginated, keyset, date-grouped — session 25, PR #31), **design-system foundation** (tokens, Geist type, lucide icons — session 26, PR #32), **drive-detail + 3-state drive health** (session 27, PR #33), **drive-telemetry charts** — the app's first live `get_drive_telemetry` read (session 28, PR #34), and this **week-close pass** — map placeholder, state-coverage polish, docs (session 29).

- **How the week diverged from its original sketch (honest record).** The original Week-4 App sketch imagined a *chart-exploration* screen: a metric picker, pinch/zoom, and a separate "multi-metric small multiples (4 metrics at once)" view — roughly a Day-3/Day-4 split. The delivered Figma (design §6 S4, §3, §10) instead specifies a **fixed trio** (Speed / Boost / Coolant, coolant peak amber) with **no bespoke chart interaction** (no picker, no pinch/zoom) and no small-multiples view. That collapsed the imagined Day-3/Day-4 chart-exploration split into **one live-wiring day against a fixed chart set**, which is both smaller and higher-value. The freed capacity went into two things not in the original Week-4 sketch: (a) the **design-system foundation session** (PR #32 — pulled forward the moment the Figma landed, so every subsequent screen builds on tokens/type/icons rather than stock palette), and (b) the **radius-namespace fix** caught in PR #32 review (the first commit had silently restyled ~23 old `rounded-xl` cards; namespaced to `rounded-ds-*` to hold strict forward-only). The **metric picker / pinch-zoom / small-multiples** items are **deliberately not built** — the design rules them out for this screen (bespoke chart interaction lives on Diagnostic detail, Week 5+), not deferred-by-slip. **Map view** shipped as an **honest placeholder** (no map dependency): GPS is only conditional in the sync architecture and no lat/lng key exists in the metric vocabulary (`TODO(gps-route)`).
...
- **Notification-preference model conflict** — the design, the schema and the App described
  "which severities notify me" three non-equivalent ways. **Resolved 2026-08-02 as Option
  C1** (keep the threshold for the ranked Warning/Info tiers, Critical becomes an always-on
  floor, `insufficient` gets its own boolean on `user_preferences`). **Still open** on one
  designer intent-check — C1 holds only if §6's four-toggle layout was a default drawing
  rather than a deliberate non-monotone requirement. See `docs/11_Carry_Forwards.md`
  § CF-38.
...
`CF-36` (seen/ack state on `dtcs` — the `diagnostic_outputs.status` shape is the one to
copy), and **CF-38's new `insufficient` boolean on `user_preferences`** (additive,
