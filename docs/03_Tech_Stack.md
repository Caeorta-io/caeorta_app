...
| Icons | lucide-react-native + lucide-react | Same icon set both platforms. Supersedes the earlier expo-symbols / SF-Symbols direction (SF Symbols are iOS-flavoured; this build is Android-only). **`expo-symbols` is imported by no App code, but it is NOT removable** — `expo-router` declares it as a hard dependency (`expo-router@56.2.8` `package.json:157`), so it installs regardless of our manifest. The direct entry in `apps/mobile/package.json` is redundant, not load-bearing; dropping it would save nothing (see `docs/11` § CF-22, closed won't-do 2026-08-03). |
